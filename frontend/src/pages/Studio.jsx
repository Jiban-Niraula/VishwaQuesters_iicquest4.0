...
export default function Studio() {
  ...
  const drawVideoFit = useCallback((ctx, video, x, y, w, h, zoom = 1) => {
    ...
    ctx.filter = "contrast(1.1) saturate(1.2) brightness(1.05) hue-rotate(-2deg)";
  ...
  const renderOverlay = useCallback((ctx, overlay) => {
    ...
    const titleWidth = overlay.title ? ctx.measureText(overlay.title.toUpperCase()).width : 0;
    ...
    if (overlay.type === "football-scorecard") {
      ...
      w = canvas.width; h = 70; // Full width bottom bar
    } else if (overlay.type === "ad" || overlay.type === "replay" || overlay.type === "image" || overlay.type === "video-link") {
      // Full screen or specific pip size
      if (overlay.position === "full") {
        w = canvas.width; h = canvas.height;
      } else {
        w = 320; h = 180; // Default 16:9 box
      }
    }
    ...
    switch (overlay.position) {
      case "top-left":
        ...
        break;
      case "top-right":
        x = canvas.width - w - 32; y = 32;
        break;
      case "bottom-left":
        x = 32; y = canvas.height - h - 32;
        break;
      case "bottom-right":
        x = canvas.width - w - 32; y = canvas.height - h - 32;
        break;
      case "center":
    ...
    if (overlay.type === "ad" || overlay.type === "replay" || overlay.type === "image") {
    ...
    if (overlay.position !== "full") {
      ...
      ctx.lineWidth = 1;
      if (ctx.roundRect) { ctx.stroke(); } else { ctx.strokeRect(x, y, w, h); }
      
      // Accent bar
      const accentGradient = ctx.createLinearGradient(x, y, x, y + h);
      accentGradient.addColorStop(0, overlay.type === "football-scorecard" ? "#10B981" : (overlay.type === "cricket-scorecard" ? "#F59E0B" : "#3B82F6"));
      accentGradient.addColorStop(1, overlay.type === "football-scorecard" ? "#059669" : (overlay.type === "cricket-scorecard" ? "#D97706" : "#8B5CF6"));
    ...
    if (overlay.type === "football-scorecard") {
      ...
      const bowlTeam = (data.bowlingTeam || "BWL").substring(0, 3).toUpperCase();
      ...
      if (bowlHistory) {
         ...
         balls.forEach(ball => {
           ...
           ctx.fillStyle = "rgba(255,255,255,0.1)";
           if (ball === "W") ctx.fillStyle = "#EF4444"; // Red for wicket
    ...
    async (stream) => {
      ...
      ws.onopen = () => {
        ...
        if (canvasRef.current) {
          canvasRef.current.captureStream(30).getVideoTracks().forEach(t => wsStream.addTrack(t));
        }
  ...
  const syncMixer = useCallback(() => {
    ...
    if (commentaryActive && commentaryStreamRef.current) {
      ...
      if (audioTracks.length > 0) {
        ...
        if (!existing || existing.stream !== stream) {
          if (existing) {
            try { existing.sourceNode.disconnect(); } catch (e) {}
          ...
          try {
            ...
            audioSourcesRef.current[sourceId] = { sourceNode, gainNode, stream };
            existing = audioSourcesRef.current[sourceId];
            console.log("🔊 Web Audio Mixer: Connected commentary mic");
          } catch (err) {
            console.error("Failed to connect commentary mic to Web Audio Mixer:", err);
          }
        }
        ...
        if (existing) {
          ...
          existing.gainNode.gain.setTargetAtTime(targetGain, audioCtx.currentTime, 0.1);
        }
      }
    }
    ...
    Object.keys(activeOverlays).forEach((overlayId) => {
      ...
      if (mediaInfo && mediaInfo.element && (mediaInfo.element.tagName === "VIDEO" || mediaInfo.element.tagName === "AUDIO")) {
        ...
        if (!existing || existing.mediaElement !== mediaElement) {
          ...
          try {
            ...
            audioSourcesRef.current[sourceId] = { sourceNode, gainNode, mediaElement };
            existing = audioSourcesRef.current[sourceId];
        ...
        if (existing) {
          existing.gainNode.gain.setTargetAtTime(1.0, audioCtx.currentTime, 0.1);
        }
      }
    });
  ...
  }, [cameras, activeAudioSource, mutedCameras, commentaryActive, commentaryMuted, activeOverlays]);

  // Sync the audio mixer whenever inputs or active settings change
  useEffect(() => {
    syncMixer();
  }, [cameras, activeAudioSource, mutedCameras, commentaryActive, commentaryMuted, activeOverlays, syncMixer]);
  ...
  const startCanvasCapture = useCallback(async () => {
    ...
    if (!hasSystemAudio && commentaryActive && commentaryStreamRef.current && commentaryStreamRef.current.getAudioTracks().length > 0) {
      hasSystemAudio = true;
    }

    console.log(`🔊 startCanvasCapture: ${mixerSourceCount} mixer source(s), hasSystemAudio=${hasSystemAudio}`);
    ...
    async (msg) => {
      ...
      const connectAudioTrackToMixer = (audioTrack) => {
        ...
        try {
          ...
          console.log(`🔊 Camera ${cameraID}: audio track directly wired to mixer`);

          // Auto-select this camera as active audio source if none chosen yet
          setActiveAudioSource((prev) => prev || cameraID);
        } catch (err) {
          console.error(`Failed to connect camera ${cameraID} audio to mixer:`, err);
        }
      };
  ...
  const handleCandidate = useCallback((msg) => {
    ...
    const pc = peerConnectionsRef.current[senderID] || viewerPeerConnectionsRef.current[senderID];
  ...
  const handleViewerReady = useCallback(async (msg) => {
    ...
    (msg) => {
      ...
      if (msg.data?.role === "camera") {
        ...
        if (activeAudioSource === msg.from) {
          const nextAudioCam = Object.values(cameras).find(c => c.id !== msg.from && c.stream?.getAudioTracks().length > 0);
          if (nextAudioCam) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const event = events.find((e) => e.unique_code === eventCode || e.code === eventCode || String(e.id) === String(eventCode));
        if (event) {
  ...
  useEffect(() => {
    ...
    const stateRef = {
      cameras, activeCameraId, activeLayout, overlays,
      activeOverlays, isLive, getResolvedSlots, drawCameraFeed,
      renderOverlay,
    };
    ...
    const render = () => {
      ...
      const { slot1: cam1, slot2: cam2, slot3: cam3, slot4: cam4 } = s.getResolvedSlots();

      if (cameraList.length > 0) {
        if (s.activeLayout === "single") {
          if (cam1) {
            s.drawCameraFeed(ctx, cam1, `${cam1.id.split("_")[0]} (Main)`, 0, 0, canvas.width, canvas.height);
          }
        } else if (s.activeLayout === "side-by-side") {
          const halfW = canvas.width / 2;
          s.drawCameraFeed(ctx, cam1, cam1 ? `${cam1.id.split("_")[0]} (Left)` : "Camera 1 (Offline)", 0, 0, halfW, canvas.height);
          s.drawCameraFeed(ctx, cam2, cam2 ? `${cam2.id.split("_")[0]} (Right)` : "Camera 2 (Offline)", halfW, 0, halfW, canvas.height);
          ctx.fillStyle = "#1E293B"; ctx.fillRect(halfW - 1, 0, 2, canvas.height);
        } else if (s.activeLayout === "pip") {
          s.drawCameraFeed(ctx, cam1, cam1 ? `${cam1.id.split("_")[0]} (Main)` : "Camera 1 (Offline)", 0, 0, canvas.width, canvas.height);
          if (cam2) {
            const pipW = canvas.width / 4, pipH = canvas.height / 4;
            const pipX = canvas.width - pipW - 20, pipY = canvas.height - pipH - 20;
            ctx.save(); ctx.shadowColor = "rgba(0,0,0,0.5)"; ctx.shadowBlur = 15;
            ctx.fillStyle = "#090D11"; ctx.fillRect(pipX - 2, pipY - 2, pipW + 4, pipH + 4); ctx.restore();
            s.drawCameraFeed(ctx, cam2, `${cam2.id.split("_")[0]} (PiP)`, pipX, pipY, pipW, pipH);
          }
        } else if (s.activeLayout === "grid") {
          const w = canvas.width / 2, h = canvas.height / 2;
          s.drawCameraFeed(ctx, cam1, cam1 ? `${cam1.id.split("_")[0]} (Cam 1)` : "Camera 1 (Offline)", 0, 0, w, h);
          s.drawCameraFeed(ctx, cam2, cam2 ? `${cam2.id.split("_")[0]} (Cam 2)` : "Camera 2 (Offline)", w, 0, w, h);
          s.drawCameraFeed(ctx, cam3, cam3 ? `${cam3.id.split("_")[0]} (Cam 3)` : "Camera 3 (Offline)", 0, h, w, h);
          s.drawCameraFeed(ctx, cam4, cam4 ? `${cam4.id.split("_")[0]} (Cam 4)` : "Camera 4 (Offline)", w, h, w, h);
          ctx.fillStyle = "#1E293B"; ctx.fillRect(w - 1, 0, 2, canvas.height); ctx.fillRect(0, h - 1, canvas.width, 2);
        } else if (s.activeLayout === "wide-cu") {
          const mainW = canvas.width * 0.75, sideW = canvas.width * 0.25, sideH = canvas.height / 2;
          s.drawCameraFeed(ctx, cam1, cam1 ? `${cam1.id.split("_")[0]} (Wide)` : "Camera 1 (Offline)", 0, 0, mainW, canvas.height);
          s.drawCameraFeed(ctx, cam2, cam2 ? `${cam2.id.split("_")[0]} (CU 1)` : "Camera 2 (Offline)", mainW, 0, sideW, sideH);
          s.drawCameraFeed(ctx, cam3, cam3 ? `${cam3.id.split("_")[0]} (CU 2)` : "Camera 3 (Offline)", mainW, sideH, sideW, sideH);
          ctx.fillStyle = "#1E293B"; ctx.fillRect(mainW - 1, 0, 2, canvas.height); ctx.fillRect(mainW, sideH - 1, sideW, 2);
        }
      } else {
      ...
      // Draw LIVE indicator
      if (s.isLive) {
        ctx.fillStyle = "rgba(239, 68, 68, 0.9)"; ctx.fillRect(16, 16, 70, 28);
    ...
    cameras, activeCameraId, activeLayout, overlays,
    activeOverlays, isLive, drawVideoFit, drawCameraFeed,
    getResolvedSlots, renderOverlay,
  ]);
  ...
  const goLive = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn("RTMP streaming disabled or failed to start. Streaming on platform only.");
      }
  ...
  const handleCreateOverlay = async () => {
    ...
    try {
      ...
      if (newOverlay.type === "football-scorecard" || newOverlay.type === "cricket-scorecard") {
      ...
      if (newOverlayMedia && (newOverlay.type === "ad" || newOverlay.type === "replay" || newOverlay.type === "image")) {
        ...
        if (newOverlayMedia.type.startsWith("video/")) {
          ...
          video.playsInline = true;
          video.load();
          mediaElementsRef.current[ov.id] = { type: "video", element: video, url };
        } else if (newOverlayMedia.type.startsWith("image/")) {
          const img = new Image();
          img.src = url;
          mediaElementsRef.current[ov.id] = { type: "image", element: img, url };
        }
      }
      ...
      if (newOverlay.type === "video-link" && newOverlayVideoUrl) {
        ...
        mediaElementsRef.current[ov.id] = { type: "video-link", element: video, url };
  ...
  const toggleOverlayActive = async (overlay) => {
    try {
      ...
      const mediaInfo = mediaElementsRef.current[overlay.id];
      if (mediaInfo && (mediaInfo.type === "video" || mediaInfo.type === "video-link")) {
        if (newState) {
  ...
  const startScreenShare = async () => {
    try {
      ...
      if (audioTracks.length > 0) {
        ...
        sourceNode.connect(gainNode);
        gainNode.connect(destNode);
        audioSourcesRef.current[screenId] = { sourceNode, gainNode, stream: audioOnlyStream };
  ...
  const stopScreenShare = (screenId) => {
    ...
    const sid = screenId || Object.keys(cameras).find((id) => cameras[id]?.type === "screen");
    if (!sid) return;
    ...
    if (audioSourcesRef.current[sid]) {
      try { audioSourcesRef.current[sid].sourceNode.disconnect(); } catch (e) {}
      try { audioSourcesRef.current[sid].gainNode.disconnect(); } catch (e) {}
  ...
  const enableLocalRecording = async () => {
    ...
    try {
      ...
      const rootHandle = await dirHandle.getDirectoryHandle("StreamAngle Recordings", { create: true });
      recordingDirHandleRef.current = rootHandle;
      setDownloadEnabled(true);
      startPerCameraRecording(rootHandle);
    } catch (err) {
      if (err.name !== "AbortError") console.error("Directory picker failed:", err);
    }
  };
  ...
  const startPerCameraRecording = async (rootHandle) => {
    ...
    for (const cam of activeCams) {
      ...
      try {
        const camName = (cam.deviceName || cam.id).replace(/[^a-z0-9_\-]/gi, "_");
        const camDirHandle = await rootHandle.getDirectoryHandle(camName, { create: true });
        ...
        const startRecordingChunk = () => {
          ...
          recorder.ondataavailable = async (e) => {
            if (e.data && e.data.size > 0 && recordingDirHandleRef.current) {
              ...
              try {
                const fileHandle = await camDirHandle.getFileHandle(filename, { create: true });
                const writable = await fileHandle.createWritable();
                await writable.write(e.data);
                await writable.close();
                console.log(`📹 Saved ${camName}/${filename} (${(e.data.size / 1024).toFixed(1)}KB)`);
              } catch (writeErr) {
        ...
        cameraRecordersIntervalsRef.current[cam.id] = setInterval(() => {
          if (cameraRecordersRef.current[cam.id] && cameraRecordersRef.current[cam.id].state !== "inactive") {
  ...
  const addDelivery = (overlay, delivery) => {
    ...
    if (delivery === "Undo") {
      if (ballLog.length > 0) {
        ...
        Object.keys(prev).forEach(k => { data[k] = prev[k]; });
      }
    } else {
      ...
      const histArr = data.bowlHistory ? data.bowlHistory.split(" ").filter(Boolean) : [];
      ...
      if (extraMatch) {
        ...
        if (isStriker1) {
          data.battingLog.push({ name: data.bat1Name, runs: data.bat1Runs, out: true });
          data.bat1Name = "New Batsman"; data.bat1Runs = 0;
        } else {
          data.battingLog.push({ name: data.bat2Name, runs: data.bat2Runs, out: true });
          data.bat2Name = "New Batsman"; data.bat2Runs = 0;
        }
        data.legalBalls += 1;
      } else {
      ...
      if (isLegal) {
        data.totalLegalBalls = (data.totalLegalBalls || Math.round(parseFloat(data.overs || 0) * 6)) + 1;
      }
      ...
      if (data.totalLegalBalls !== undefined) {
        ...
        data.overs = parseFloat((teamCompletedOvers + teamBallsThisOver / 10).toFixed(1));
      } else {
        data.overs = parseFloat((data.bowlOvers + data.legalBalls / 10).toFixed(1));
      }
  ...
  const handlePlaySponsoredAd = async (item) => {
    ...
    try {
      ...
      const payout = result.earnedAmount || result.earned_amount || item.yourPayout || ad.creatorPayoutPro || ad.creatorPayoutFree || 0;
      const overlayId = `sponsored-${placement.id || Date.now()}`;
      const mediaURL = resolveMediaUrl(ad.mediaUrl || ad.media_url || ad.videoUrl || ad.video_url || ad.imageUrl || ad.image_url);
      const type = ad.type === "image" ? "image" : "ad";
      const element = ad.type === "image" ? new Image() : document.createElement("video");
      ...
      if (ad.type !== "image") {
        ...
        element.loop = false;
        element.onended = () => handleCompleteSponsoredAd(overlayId, placement, ad);
        element.play().catch(() => {});
      }
      ...
      setOverlays((prev) => [...prev.filter((o) => o.id !== overlayId), overlay]);
      setActiveOverlays((prev) => ({ ...prev, [overlayId]: true }));
      setCurrentSponsoredAd({ overlayId, placement, ad, payout, startedAt: Date.now() });
    } catch (err) {
  ...
  const handleCompleteSponsoredAd = async (overlayId, placementArg, adArg) => {
    const active = currentSponsoredAd || { overlayId, placement: placementArg, ad: adArg, startedAt: Date.now() };
    if (!active?.placement?.id) return;

    const watchedSeconds = active.ad?.type === "video"
      ? (active.ad.durationSeconds || active.ad.duration_seconds || Math.max(1, Math.round((Date.now() - active.startedAt) / 1000)))
    ...
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={downloadEnabled ? disableLocalRecording : enableLocalRecording}
            title={downloadEnabled ? "Stop local recording" : "Enable local 10s clip recording"}
          ...
          >
            {downloadEnabled ? "🔴 REC" : "⏺ Record Clips"}
          </button>
          {/* Screen Share Button */}
          <button
            onClick={isScreenSharing ? () => stopScreenShare(null) : startScreenShare}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-80 bg-gray-900 border-l border-gray-800 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4">
                    ...
                    {cam.type !== "screen" && (
                      <div className="mt-2 flex items-center gap-2">
                        ...
                        <input
                          ...
                          onChange={(e) => setCameraZoom(p => ({ ...p, [cam.id]: parseFloat(e.target.value) }))}
                          className="flex-1 accent-red-500"
                        />
                        <span className="text-xs text-gray-400 w-6">{(cameraZoom[cam.id] || 1).toFixed(1)}x</span>
                      </div>
                    )}
                  </div>
                ))}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between gap-2 text-xs">
                        ...
                        >
                          ...
                          {Object.values(cameras).map(cam => (
                            <option key={cam.id} value={cam.id}>{(cam.deviceName || cam.id.split("_")[0])}</option>
                          ))}
                        </select>
                      </div>
                  ...
                  {newOverlay.type === "football-scorecard" && (
                    <div className="space-y-2">
                      <input
                        ...
                        onChange={(e) => setScorecardData(p => ({ ...p, teamA: e.target.value, scoreA: 0 }))}
                        className="w-full bg-gray-700 text-white rounded px-3 py-2 text-sm"
                      />
                      <input
                        type="text"
                        placeholder="Team B Name"
                        onChange={(e) => setScorecardData(p => ({ ...p, teamB: e.target.value, scoreB: 0 }))}
                        className="w-full bg-gray-700 text-white rounded px-3 py-2 text-sm"
                      />
                    </div>
                  )}
                  {newOverlay.type === "cricket-scorecard" && (
                    <div className="space-y-2">
                      <input
                        ...
                        onChange={(e) => setScorecardData(p => ({ ...p, battingTeam: e.target.value, runs: 0, wickets: 0, overs: 0, bat1Name: "Player 1", bat1Runs: 0, bat1Strike: true, bat2Name: "Player 2", bat2Runs: 0, bat2Strike: false, bowlName: "Bowler 1", bowlOvers: 0, bowlRuns: 0, bowlWickets: 0, bowlHistory: "" }))}
                        className="w-full bg-gray-700 text-white rounded px-3 py-2 text-sm"
                      />
                      <input
                        type="text"
                        placeholder="Bowling Team"
                        onChange={(e) => setScorecardData(p => ({ ...p, bowlingTeam: e.target.value }))}
                        className="w-full bg-gray-700 text-white rounded px-3 py-2 text-sm"
                      />
                    </div>
                  )}
                  {(newOverlay.type === "ad" || newOverlay.type === "replay" || newOverlay.type === "image") && (
                    <input
                      type="file"
                      accept="image/*,video/*"
                      onChange={(e) => {
                  ...
                  {newOverlay.type === "video-link" && (
                    <div className="space-y-2">
                      <input
                        ...
                        onChange={(e) => setNewOverlayVideoUrl(e.target.value)}
                        className="w-full bg-gray-700 text-white rounded px-3 py-2 text-sm"
                      />
                      {(newOverlayVideoUrl.includes("youtube.com") || newOverlayVideoUrl.includes("youtu.be")) && (
                        <div className="text-xs bg-amber-500/10 text-amber-300 p-3 rounded border border-amber-500/30 leading-relaxed">
                          ⚠️ <b>YouTube Security Restriction</b>
                          <br />
                          Browsers forbid broadcasting YouTube streams directly inside a studio canvas due to security rules.
                          <br /><br />
                          <span className="text-white font-medium">Workaround:</span> Open the YouTube video in a new browser tab, and use the <b>🖥️ Share Screen</b> button at the top of the studio to bring it into your broadcast!
                        </div>
                      )}
                    </div>
                  )}
            ...
            {activePanel === "overlays" && overlays.filter(o => activeOverlays[o.id] && (o.type === 'football-scorecard' || o.type === 'cricket-scorecard')).length > 0 && (
              <div className="px-4 pb-4 space-y-4">
                <h3 className="font-bold text-sm text-gray-400 uppercase mt-4 border-t border-gray-800 pt-4">
                  Scorecard Controls
                </h3>
                {overlays.filter(o => activeOverlays[o.id] && (o.type === 'football-scorecard' || o.type === 'cricket-scorecard')).map(overlay => {
                  const data = overlay.content ? JSON.parse(overlay.content) : {};
                  return (
                    <div key={`controls-${overlay.id}`} className="bg-gray-800 rounded-lg p-3 space-y-3">
                      <div className="text-sm font-bold text-white mb-2">{overlay.title}</div>
                      {overlay.type === 'football-scorecard' && (
                        <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1">
                            <div className="text-xs text-gray-400">{data.teamA || "Team A"} Score</div>
                            <div className="flex gap-1">
                              <button onClick={() => { data.scoreA = (data.scoreA || 0) + 1; updateOverlay(overlay.id, { content: JSON.stringify(data) }).then(() => { overlay.content = JSON.stringify(data); setOverlays([...overlays]); }) }} className="flex-1 bg-gray-700 hover:bg-gray-600 rounded py-1 text-sm">+</button>
                              <button onClick={() => { data.scoreA = Math.max(0, (data.scoreA || 0) - 1); updateOverlay(overlay.id, { content: JSON.stringify(data) }).then(() => { overlay.content = JSON.stringify(data); setOverlays([...overlays]); }) }} className="flex-1 bg-gray-700 hover:bg-gray-600 rounded py-1 text-sm">-</button>
                            </div>
                          </div>
                          <div className="space-y-1">
                            <div className="text-xs text-gray-400">{data.teamB || "Team B"} Score</div>
                            <div className="flex gap-1">
                              <button onClick={() => { data.scoreB = (data.scoreB || 0) + 1; updateOverlay(overlay.id, { content: JSON.stringify(data) }).then(() => { overlay.content = JSON.stringify(data); setOverlays([...overlays]); }) }} className="flex-1 bg-gray-700 hover:bg-gray-600 rounded py-1 text-sm">+</button>
                              <button onClick={() => { data.scoreB = Math.max(0, (data.scoreB || 0) - 1); updateOverlay(overlay.id, { content: JSON.stringify(data) }).then(() => { overlay.content = JSON.stringify(data); setOverlays([...overlays]); }) }} className="flex-1 bg-gray-700 hover:bg-gray-600 rounded py-1 text-sm">-</button>
                            </div>
                          </div>
                        </div>
                      )}
                      {overlay.type === 'cricket-scorecard' && (() => {
                        const d = overlay.content ? JSON.parse(overlay.content) : {};
                        const bat1Strike = d.bat1Strike !== false;
                        const deliveryBtn = (label, delivery, color = "bg-gray-700 hover:bg-gray-600") => (
                          <button
                            key={label}
                            onClick={() => addDelivery(overlay, delivery)}
                        ...
                        const updateField = (field, value) => {
                          d[field] = value;
                          updateOverlay(overlay.id, { content: JSON.stringify(d) }).then(() => {
                        ...
                        return (
                          <div className="space-y-3">
                            {/* Batsmen Names */}
                            <div className="space-y-1">
                              <div className="text-xs text-gray-400 uppercase font-semibold">Batsmen</div>
                              <div className="flex gap-2 items-center">
                                <span className={`w-2 h-2 rounded-full shrink-0 ${bat1Strike ? 'bg-green-400' : 'bg-gray-600'}`} />
                                <input
                                  type="text"
                                  value={d.bat1Name || ""}
                                  onChange={e => updateField('bat1Name', e.target.value)}
                                  placeholder="Batsman 1"
                                  className="flex-1 bg-gray-700 text-white rounded px-2 py-1 text-sm"
                                />
                                <span className="text-amber-400 font-bold text-sm w-8 text-center">{d.bat1Runs || 0}</span>
                                <button
                                  onClick={() => { d.bat1Strike = true; d.bat2Strike = false; updateField('bat1Strike', true); }}
                              ...
                              <div className="flex gap-2 items-center">
                                ...
                                <input
                                  type="text"
                                  value={d.bat2Name || ""}
                                  onChange={e => updateField('bat2Name', e.target.value)}
                                  placeholder="Batsman 2"
                                  className="flex-1 bg-gray-700 text-white rounded px-2 py-1 text-sm"
                                />
                                <span className="text-amber-400 font-bold text-sm w-8 text-center">{d.bat2Runs || 0}</span>
                                <button
                                  onClick={() => { d.bat2Strike = true; d.bat1Strike = false; updateField('bat2Strike', true); }}
                                  className={`text-xs px-2 py-1 rounded ${!bat1Strike ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-400'}`}
                                >
                                  *
                                </button>
                            ...
                            {/* Bowler */}
                            <div className="space-y-2">
                              <div className="text-xs text-gray-400 uppercase font-semibold">Bowler</div>
                              {/* Current bowler name + figures */}
                              <div className="flex gap-2 items-center">
                                <input
                                  type="text"
                                  value={d.bowlName || ""}
                                  onChange={e => updateField('bowlName', e.target.value)}
                                  placeholder="Bowler Name"
                                  className="flex-1 bg-gray-700 text-white rounded px-2 py-1 text-sm"
                                />
                                <span className="text-xs text-amber-400 font-mono font-bold whitespace-nowrap">{d.bowlOvers || 0}-{d.bowlWickets || 0}-{d.bowlRuns || 0}</span>
                              </div>

                              {/* Change Bowler panel */}
                              <div className="bg-gray-900 rounded p-2 space-y-2">
                                <div className="text-xs text-gray-500">Change bowler →</div>
                                ...
                                {(d.bowlingLog || []).length > 0 && (
                                  <select
                                    ...
                                    onChange={e => {
                                      ...
                                      const existing = [...(d.bowlingLog || [])];
                                      const idx = existing.findIndex(b => b.name === d.bowlName);
                                      if (idx >= 0) {
                                        existing[idx] = { ...existing[idx], overs: existing[idx].overs + (d.bowlOvers || 0), wickets: existing[idx].wickets + (d.bowlWickets || 0), runs: existing[idx].runs + (d.bowlRuns || 0) };
                                      } else if (d.bowlName) {
                                        existing.push({ name: d.bowlName, overs: d.bowlOvers || 0, wickets: d.bowlWickets || 0, runs: d.bowlRuns || 0 });
                                      }
                                      // Restore selected previous bowler's accumulated stats
                                      const sel = existing.find(b => b.name === e.target.value) || {};
                                      const remaining = existing.filter(b => b.name !== e.target.value);
                                      d.bowlingLog  = remaining;
                                      d.bowlName    = sel.name || e.target.value;
                                  ...
                                  >
                                    <option value="">↩ Return a previous bowler…</option>
                                    {(d.bowlingLog || []).map((b, i) => (
                                      <option key={i} value={b.name}>{b.name} ({b.overs}-{b.wickets}-{b.runs})</option>
                                    ))}
                                  </select>
                                )}
                                ...
                                <div className="flex gap-1">
                                  ...
                                  <button
                                    onClick={() => {
                                      const input = document.getElementById(`new-bowler-${overlay.id}`);
                              ...
                              {(d.bowlingLog || []).length > 0 && (
                                <div className="bg-gray-900 rounded p-2 space-y-1">
                                  <div className="text-xs text-gray-500 uppercase mb-1">Bowling Log (O-W-R)</div>
                                  {(d.bowlingLog || []).map((b, i) => (
                                    <div key={i} className="flex justify-between text-xs">
                                      <span className="text-gray-300">{b.name}</span>
                                      <span className="text-amber-400 font-mono">{b.overs}-{b.wickets}-{b.runs}</span>
                                    </div>
                                  ))}
                                </div>
                              )}
                            ...
                            {d.bowlHistory && (
                              <div className="flex gap-1 flex-wrap">
                                <span className="text-xs text-gray-500 mr-1 self-center">This over:</span>
                                {d.bowlHistory.split(' ').filter(Boolean).map((ball, i) => (
                                  <span
                                    key={i}
                            ...
                            <div className="bg-gray-900 rounded p-2 flex justify-between text-sm">
                              <span className="text-white font-bold">{d.runs || 0}-{d.wickets || 0}</span>
                              <span className="text-gray-400">Overs: {d.overs || '0.0'}</span>
                              <span className="text-xs text-gray-500">{d.legalBalls || 0}/6 balls</span>
                            </div>

                            {/* Runs Buttons */}
                            <div>
                              <div className="text-xs text-gray-400 mb-1 uppercase font-semibold">Runs</div>
                              <div className="grid grid-cols-6 gap-1">
                                {deliveryBtn("0", "0")}
                                {deliveryBtn("1", "1")}
                                {deliveryBtn("2", "2")}
                                {deliveryBtn("3", "3")}
                                {deliveryBtn("4", "4", "bg-blue-700 hover:bg-blue-600")}
                                {deliveryBtn("6", "6", "bg-blue-700 hover:bg-blue-600")}
                              </div>
                            </div>

                            {/* Extras */}
                            <div className="space-y-2">
                              <div className="text-xs text-gray-400 mb-1 uppercase font-semibold">Extras</div>

                              {/* Wide */}
                              <div className="flex items-center gap-1">
                                <span className="text-xs text-yellow-400 w-16 shrink-0 font-semibold">Wide</span>
                                <div className="grid grid-cols-6 gap-1 flex-1">
                                  {deliveryBtn("Wd",  "Wd",   "bg-yellow-700 hover:bg-yellow-600")}
                                  {deliveryBtn("+1",  "Wd+1", "bg-yellow-700 hover:bg-yellow-600")}
                                  {deliveryBtn("+2",  "Wd+2", "bg-yellow-700 hover:bg-yellow-600")}
                                  {deliveryBtn("+3",  "Wd+3", "bg-yellow-700 hover:bg-yellow-600")}
                                  {deliveryBtn("+4",  "Wd+4", "bg-yellow-700 hover:bg-yellow-600")}
                                  {deliveryBtn("+5",  "Wd+5", "bg-yellow-700 hover:bg-yellow-600")}
                                </div>
                              </div>

                              {/* No Ball */}
                              <div className="flex items-center gap-1">
                                <span className="text-xs text-orange-400 w-16 shrink-0 font-semibold">No Ball</span>
                                <div className="grid grid-cols-6 gap-1 flex-1">
                                  {deliveryBtn("Nb",  "Nb",   "bg-orange-700 hover:bg-orange-600")}
                                  {deliveryBtn("+1",  "Nb+1", "bg-orange-700 hover:bg-orange-600")}
                                  {deliveryBtn("+2",  "Nb+2", "bg-orange-700 hover:bg-orange-600")}
                                  {deliveryBtn("+3",  "Nb+3", "bg-orange-700 hover:bg-orange-600")}
                                  {deliveryBtn("+4",  "Nb+4", "bg-orange-700 hover:bg-orange-600")}
                                  {deliveryBtn("+6",  "Nb+6", "bg-orange-700 hover:bg-orange-600")}
                                </div>
                              </div>

                              {/* Leg Bye */}
                              <div className="flex items-center gap-1">
                                <span className="text-xs text-blue-300 w-16 shrink-0 font-semibold">Leg Bye</span>
                                <div className="grid grid-cols-4 gap-1 flex-1">
                                  {deliveryBtn("Lb+1", "Lb+1", "bg-slate-600 hover:bg-slate-500")}
                                  {deliveryBtn("Lb+2", "Lb+2", "bg-slate-600 hover:bg-slate-500")}
                                  {deliveryBtn("Lb+3", "Lb+3", "bg-slate-600 hover:bg-slate-500")}
                                  {deliveryBtn("Lb+4", "Lb+4", "bg-slate-600 hover:bg-slate-500")}
                                </div>
                              </div>

                              {/* Bye */}
                              <div className="flex items-center gap-1">
                                <span className="text-xs text-purple-300 w-16 shrink-0 font-semibold">Bye</span>
                                <div className="grid grid-cols-4 gap-1 flex-1">
                                  {deliveryBtn("B+1",  "B+1",  "bg-purple-800 hover:bg-purple-700")}
                                  {deliveryBtn("B+2",  "B+2",  "bg-purple-800 hover:bg-purple-700")}
                                  {deliveryBtn("B+3",  "B+3",  "bg-purple-800 hover:bg-purple-700")}
                                  {deliveryBtn("B+4",  "B+4",  "bg-purple-800 hover:bg-purple-700")}
                                </div>
                              </div>
                            </div>
                            ...
                            {/* Wicket + Undo */}
                            <div className="grid grid-cols-2 gap-2">
                              {deliveryBtn("⚡ WICKET", "W", "bg-red-700 hover:bg-red-600 col-span-1")}
                              {deliveryBtn("↩ Undo", "Undo", "bg-gray-600 hover:bg-gray-500 col-span-1")}
                            </div>
                            ...
                            <button
                              onClick={() => {
                                const allBatsmen = [
                                  ...(d.battingLog || []),
                                  { name: d.bat1Name || 'Batsman 1', runs: d.bat1Runs || 0, out: false },
                                  { name: d.bat2Name || 'Batsman 2', runs: d.bat2Runs || 0, out: false },
                                ];
                                const allBowlers = [
                                  ...(d.bowlingLog || []),
                                  { name: d.bowlName || 'Bowler', overs: d.bowlOvers || 0, wickets: d.bowlWickets || 0, runs: d.bowlRuns || 0 },
                                ];
                                const lines = [
                                  '===== MATCH SCORECARD =====',
                                  ...
                                  '--- BATTING ---',
                                  'Batsman                 Runs  Status',
                                  ...allBatsmen.map(b => `${(b.name || '').padEnd(24)}${String(b.runs || 0).padEnd(6)}${b.out ? 'Out' : 'Not Out'}`),
                                  '',
                                  '--- BOWLING ---',
                                  'Bowler                  Overs  Wkts  Runs',
                                  ...allBowlers.map(b => `${(b.name || '').padEnd(24)}${String(b.overs || 0).padEnd(7)}${String(b.wickets || 0).padEnd(6)}${b.runs || 0}`),
                                  '',
            ...
            {activePanel === "audio" && (
              <div className="space-y-4">
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col">
                        <span className="text-sm font-medium">{(cam.deviceName || cam.id.split("_")[0])}</span>
                        <span className="text-xs mt-1">
                          {!hasMic ? (
                            <span className="text-gray-500">No mic input</span>
                          ) : isMuted ? (
                            <span className="text-red-400 font-semibold">🔇 Muted</span>
                          ) : isActive ? (
                            <span className="text-green-400 font-bold">🔊 Main Voice (Highlighted)</span>
                          ) : (
                            <span className="text-indigo-400">🔉 Ducked (Background)</span>
                          )}
                        </span>
                      </div>
                      {hasMic && (
                ...
                {commentaryActive ? (
                  <div className={`bg-gray-800 rounded-lg p-3 space-y-2 border transition ${activeAudioSource === "commentary" && !commentaryMuted ? "border-green-500/50" : "border-transparent"}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="text-sm font-medium">🎤 Commentary Mic</span>
                        <span className="text-xs mt-1">
                          {commentaryMuted ? (
                            <span className="text-red-400 font-semibold">🔇 Muted</span>
            ...
            {activePanel === "sponsored" && (
              <div className="space-y-4">
                <h3 className="font-bold text-sm text-gray-400 uppercase">Sponsored Ads Marketplace</h3>
                {currentSponsoredAd && (
                  <div className="bg-green-900/40 border border-green-700 rounded-lg p-3 space-y-2">
                    <p className="text-sm font-bold text-green-200">Playing: {currentSponsoredAd.ad.title}</p>
                    <p className="text-xs text-green-100">Complete after playback to credit creator wallet.</p>
                    <button
                ...
                {sponsoredAds.length === 0 ? (
                  ...
                  sponsoredAds.map((item) => {
                    const ad = item.ad || item;
                    const mediaURL = resolveMediaUrl(ad.mediaUrl || ad.media_url);
                    return (
                      <div key={ad.id} className="bg-gray-800 rounded-lg p-3 space-y-2">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <p className="font-bold text-sm text-white">{ad.title}</p>
                            <p className="text-xs text-gray-400 capitalize">{ad.type} • payout NRS {item.yourPayout || ad.creatorPayoutPro || ad.creatorPayoutFree || 0}</p>
                          </div>
                          <span className="text-xs bg-gray-700 rounded px-2 py-1">{ad.durationSeconds || ad.duration_seconds || 0}s</span>
                        </div>
                        {mediaURL && (ad.type === "image" ? (
                          <img src={mediaURL} className="w-full h-24 object-cover rounded border border-gray-700" />
                        ) : (
                          <video src={mediaURL} className="w-full h-24 object-cover rounded border border-gray-700" muted controls />
                        ))}
                        <button
                          disabled={sponsoredAdLoading || !!currentSponsoredAd}
