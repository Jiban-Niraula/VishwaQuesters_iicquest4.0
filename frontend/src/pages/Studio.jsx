...
export default function Studio() {
	...
	const getMotionScore = useCallback((cameraId, videoElement) => {
		...
		try {
			...
			prevFrameDataRef.current[cameraId] = new Uint8ClampedArray(
				currentFrame,
			);
	...
	const setupAnalyserForCamera = useCallback((cameraId) => {
		...
		try {
			...
			console.warn(
				`AI Director could not attach analyser for ${cameraId}:`,
				err,
			);
		}
	}, []);
	...
	const initAiDirectorModel = useCallback(async () => {
		...
		try {
			...
			const vision =
				await FilesetResolver.forVisionTasks(AI_DIRECTOR_WASM_URL);
			...
			console.warn(
				"AI Director model unavailable. Falling back to audio + motion.",
				err,
			);
	...
	const getFacePresenceScore = useCallback(async (cameraId, videoElement) => {
		if (
			!videoElement ||
			videoElement.readyState < HTMLMediaElement.HAVE_CURRENT_DATA ||
			!videoElement.videoWidth ||
		...
		try {
			...
			detections.forEach((detection) => {
				...
				const score = clamp01(
					confidence * 0.75 + Math.min(faceArea * 6, 1) * 0.25,
				);
	...
	const buildDirectorScore = useCallback(
		async (cam, mode) => {
			...
			const faceScore =
				mode === "director"
					? await getFacePresenceScore(cam.id, cam.videoElement)
					: 0;
	...
	const runAutoSwitch = useCallback(async () => {
		...
		try {
			...
			const activeScore = currentActiveId
				? nextScores[currentActiveId]?.total || 0
				: 0;
	...
	const renderOverlay = useCallback((ctx, overlay) => {
		...
		if (overlay.type === "video-link") {
			...
			if (
				mediaInfo &&
				mediaInfo.element &&
				mediaInfo.element.readyState >= 2
			) {
				if (overlay.position !== "full") {
		...
		if (overlay.type === "football-scorecard") {
			...
			const data = overlay.content ? JSON.parse(overlay.content) : {};
			const batTeam = (data.battingTeam || "BAT")
				.substring(0, 3)
				.toUpperCase();
			...
			ctx.textBaseline = "middle";
			const textX =
				overlay.position === "full" ? x + w / 2 : x + padding + 4;
	...
	const startWebSocketStreaming = useCallback(
		async (stream) => {
			...
			const activeDests = destinations.filter(
				(d) => d.is_active || d.isActive,
			);
			...
			if (
				...
				if (!existing || existing.mediaElement !== mediaElement) {
					...
					try {
						...
						existing = audioSourcesRef.current[sourceId];
						console.log(
							`🔊 Connected overlay media ${overlayId} to mixer`,
						);
					} catch (err) {
		...
		Object.keys(audioSourcesRef.current).forEach((sourceId) => {
			if (!activeSources.has(sourceId)) {
				...
				if (existing) {
					...
					if (
						sourceId !== "commentary" &&
						!sourceId.startsWith("overlay_")
					) {
						cleanupAutoSwitchForCamera(sourceId);
					}
					console.log(
						`🔊 Web Audio Mixer: Disconnected source ${sourceId}`,
					);
				}
			}
		});
	...
	const handleOffer = useCallback(
		async (msg) => {
			...
			const connectAudioTrackToMixer = (audioTrack) => {
				if (!audioContextRef.current) {
					...
					try {
						...
						console.log(
							"🔊 Web Audio Mixer initialized from camera track",
						);
					} catch (err) {
				...
				try {
					const sourceNode =
						audioCtx.createMediaStreamSource(audioOnlyStream);
					const gainNode = audioCtx.createGain();
			...
			pc.onconnectionstatechange = () => {
				console.log(
					`Camera ${cameraID} connectionState:`,
					pc.connectionState,
				);
						...
						if (
							...
							if (!iceRestartAttempted) {
								...
								try {
									...
									console.warn(
										`ICE restart failed for ${cameraID}:`,
										err,
									);
								}
			...
			if (msg.data?.role === "camera") {
				...
				if (activeAudioSource === msg.from) {
					const nextAudioCam = Object.values(cameras).find(
						(c) =>
							c.id !== msg.from && c.stream?.getAudioTracks().length > 0,
					);
					if (nextAudioCam) {
				...
				const startRecordingChunk = () => {
					...
					recorder.ondataavailable = async (e) => {
						if (
							e.data &&
							e.data.size > 0 &&
							recordingDirHandleRef.current
						) {
							const filename = `clip_${Date.now()}.${selectedFormat.ext}`;
							try {
								const fileHandle = await camDirHandle.getFileHandle(
									filename,
									{
										create: true,
									},
