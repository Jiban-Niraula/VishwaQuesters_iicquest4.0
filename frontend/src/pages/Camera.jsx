...
const CAMERA_TYPES = [
  { id: 'phone',  label: 'Phone Camera',       icon: 'fa-solid fa-mobile-screen-button' },
...
export default function Camera() {
  ...
  const { isConnected, send, on, off } = useWebSocket(joined ? sessionCode : null, 'camera', cameraType);
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode: { exact: facingMode }, width: { ideal: 1920 }, height: { ideal: 1080 }, frameRate: { ideal: 30, min: 24 } },
      ...
      const msg = err.name === 'NotAllowedError' ? 'Camera access denied. Allow permissions and try again.'
  ...
  const createOffer = async (stream) => {
    ...
    try {
      ...
      pc.onicecandidate = (e) => { if (e.candidate) send('candidate', { candidate: e.candidate }); };
      pc.onconnectionstatechange = () => {
        if (pc.connectionState === 'failed') setCameraError('Connection to studio failed. Please try again.');
      };
      ...
      pc.getSenders().forEach((sender) => {
        ...
        if (sender.track?.kind === 'video') { params.encodings[0].maxBitrate = 6_000_000; params.encodings[0].maxFramerate = 30; }
        else if (sender.track?.kind === 'audio') params.encodings[0].maxBitrate = 160_000;
        sender.setParameters(params).catch(() => {});
      });
      const deviceName = streamRef.current?.getVideoTracks()[0]?.label || cameraType;
      send('offer', { offer, camera_type: cameraType, device_name: deviceName });
      peerRef.current = pc;
    } catch {
  ...
  useEffect(() => {
    const handleAnswer = async (msg) => {
      if (msg.data.answer && peerRef.current) {
        ...
        for (const c of iceCandidateQueue.current) peerRef.current.addIceCandidate(c).catch(() => {});
    ...
    const handleCandidate = (msg) => {
      if (msg.data.candidate && peerRef.current) {
        ...
        peerRef.current.remoteDescription?.type ? peerRef.current.addIceCandidate(c).catch(() => {}) : iceCandidateQueue.current.push(c);
      }
    };
    const handleAudioControl = (msg) => {
      if (msg.data.muted !== undefined && streamRef.current) {
        streamRef.current.getAudioTracks().forEach((t) => { t.enabled = !msg.data.muted; });
    ...
    return () => { off('answer', handleAnswer); off('candidate', handleCandidate); off('audio_control', handleAudioControl); };
  }, [on, off]);

  useEffect(() => {
    if (isConnected && streamRef.current && !peerRef.current) createOffer(streamRef.current);
  ...
  const handleJoin = async () => {
    if (!sessionCode.trim()) { setCameraError('Enter a session code.'); return; }
  ...
  const handleDisconnect = () => {
    ...
    setJoined(false); setCameraError('');
  };
        ...
        <div className="vc-camera-join">
          <div className="vc-camera-join-card">
            ...
            <div className="vc-camera-form">
              ...
              >
                {isRequesting
                  ? <><i className="fa-solid fa-spinner fa-spin" /> Requesting camera…</>
                  : <><i className="fa-solid fa-tower-broadcast" /> Connect to Studio</>}
              </button>

              <button type="button" className="vc-camera-back" onClick={() => navigate('/')}>
      ...
      <div className="vc-camera-live">
        ...
        <div className="vc-camera-status-bar">
          <div className="vc-camera-status-left">
            ...
            <span className="vc-camera-type-pill">{CAMERA_TYPES.find((c) => c.id === cameraType)?.label}</span>
          </div>
          <span className="vc-camera-session">Session: <strong>{sessionCode}</strong></span>
        </div>
        ...
        <div className="vc-camera-viewport">
          <video
            ...
            style={{ transform: `${facingMode === 'user' ? 'scaleX(-1) ' : ''}scale(${zoom})`, transformOrigin: 'center' }}
          />
          {!isConnected && (
        ...
        <div className="vc-camera-zoom-bar">
          ...
          <input
            ...
            onChange={(e) => { const z = parseFloat(e.target.value); setZoom(z); send('camera_zoom', { zoom: z }); }}
          />
        ...
        <div className="vc-camera-controls">
          ...
          <button type="button" className="vc-ctrl-btn" onClick={switchCamera} title="Switch camera">
            <i className="fa-solid fa-rotate" />
            <span>Flip</span>
          </button>

          <button type="button" className="vc-ctrl-btn danger" onClick={handleDisconnect} title="Disconnect">
