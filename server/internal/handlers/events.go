...
func CreateEvent(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		event := models.Event{CreatorID: userID, Title: req.Title, Description: req.Description, Code: generateUniqueCode(), IsLive: false}
...
func GetEvent(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"event": event, "cameraLimit": services.MaxCamerasForPlan(db, plan), "creatorPlan": plan})
	}
}
...
func UpdateEvent(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if input.CameraCount != nil {
			plan := services.GetCreatorPlan(db, userID)
			if !services.CanConnectCamera(db, plan, *input.CameraCount) {
